...
The first way to integrate OpenAPI schemas is by placing all your OpenAPI schema files in a folder, and then initializing your agent with the `schemas_folder` parameter. Nexus will then automatically scan this folder and convert any OpenAPI schemas it finds into `BaseTool` instances.
